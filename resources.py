...
###path = "//var//www//appdev.thestockwatchlist.com//html//images//avatars//"
...
ddbb = mongo_client["tradingfishery1"]
...
def sendEmailToAdmin(token):
    ...
    conn1 = http.client.HTTPSConnection("api.sendgrid.com")
    print("d")
    payload = "{\"personalizations\":[{\"to\":[{\"email\":\"ch109437999@gmail.com\",\"name\":\"C Hero\"}],\"subject\":\"Message From Contact Us Page\"}],\"content\": [{\"type\": \"text/plain\", \"value\": \""+str(token)+"\"}],\"from\":{\"email\":\"activation@thestockwatchlist.com\",\"name\":\"TheStockwatchlist.com\"},\"reply_to\":{\"email\":\"ch109437999@gmail.com\",\"name\":\"John Doe\"}}"
    payload1 = "{\"personalizations\":[{\"to\":[{\"email\":\"meirhasin@gmail.com\",\"name\":\"Meir Hasin\"}],\"subject\":\"Message From Contact Us Page\"}],\"content\": [{\"type\": \"text/plain\", \"value\": \""+str(token)+"\"}],\"from\":{\"email\":\"activation@thestockwatchlist.com\",\"name\":\"TheStockwatchlist.com\"},\"reply_to\":{\"email\":\"meirhasin@gmail.com\",\"name\":\"John Doe\"}}"    
#    payload = "{\"personalizations\":[{\"to\":[{\"email\":\"meirhasin@gmail.com\",\"name\":\"Meir Hasin\"}],\"subject\":\"Verification code for sign up of Thestockwatchlist.com\"}],\"content\": [{\"type\": \"text/plain\", \"value\": \""+str(token)+"\"}],\"from\":{\"email\":\"activation@thestockwatchlist.com\",\"name\":\"TheStockwatchlist.com\"},\"reply_to\":{\"email\":\"meirhasin@gmail.com\",\"name\":\"John Doe\"}}"    
    print(payload)
    headers = {
        'authorization': "Bearer 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",
    ...
    def post(self):
        try:
            ...
            if current_user.count() == 0:
                response = jwt.encode({'error' : 'User not in DB. Register as a new user'}, JWT_SECRET_KEY)
                return {
                    'result' : str(response)
                }
...
class GetWatchlist(Resource):
    def post(self):
        try:
            ...
            user_watchlist = [watchlist for watchlist in user_watchlistArray]
        ...
        try:
            ...
            for earningData in earningArray:
                ...
                if delta == 0 or delta == 1:
                    ...
                    # else:
                    #     bufdata['imageUrl'] = "/images/avatars/null_symbol.png"
                    bufdata['imageUrl'] = "https://financialmodelingprep.com/image-stock/"+ earningData['symbol'] +".jpg"
                    bufdata['id'] = i
                    datas.append(bufdata)
                else:
                    break
            sTime = datetime.now()
            print("getearningstocktime", sTime - fTime)
            return {
                'result': 'ok',
                'timestamp' : datetime.now().timestamp(),
                'data' : datas
...
class GetTopStocks(Resource):
    def get(self):
        try:
            ...
            j=0
            for stock in stocks:
                print('stock', stock['_id'])
###            for stock in stocks['result']:
                i = i + 1
                bufdata = dict()
                bufdata['id'] = i
            ...
            for stock in weekstocks:
                print('stock', stock['_id'])
###            for stock in weekstocks['result']:
                j = j + 1
                bufdata = dict()
                bufdata['id'] = j
                bufdata['name'] = stock['_id']
...
class GetChatForNotification(Resource):
    def post(self):
        try:
            ...
            chatNotification = col.aggregate(pipeline)
